...
def navigate_to_target_vtol(master, tgt_lat, tgt_lon, alt_rel_m,
    ...
    while True:
        ...
        if elapsed > timeout_s:
            print(f"Navigation timeout ({elapsed:.0f}s) proceeding with hold at current position")
            return False
...
def upload_loiter_unlim(master, lat_deg, lon_deg, alt_rel_m=ALT_REL_M,
                        gcs_keepalive_fn=None):
    """
    Upload une mission LOITER_UNLIM sur la position donnee et passe en AUTO.

    Utilisa pour le hold statique au-dessus de la cible sans jamais quitter AUTO.
    Le drone continue a loitrer en AUTO jusqu'a ce que la FSM decide de rentrer.
    """
    from arm_pipeline import set_mode_and_confirm
...
def _upload_items(master, items, timeout=15, keepalive_fn=None):
    ...
    keepalive_fn : callable optionnel appele a chaque iteration pour envoyer
    le heartbeat GCS et eviter le failsafe FS_GCS_ENABL pendant l'upload.
    """
    ...
    while time.time() - t0 < timeout:
        # Heartbeat GCS toutes les 0.5s pendant l'upload pour eviter FS_GCS_ENABL
        ...
        if mtype in ("MISSION_REQUEST", "MISSION_REQUEST_INT"):
            ...
            if getattr(msg, "type", -1) == 0:
                ...
                print(f"Mission upload ACK type={msg.type}")
                return False

    print(f"Mission upload timeout (sent up to seq={last_sent})")
    return False
